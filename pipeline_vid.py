...
def change_street_perspective(img,mtx,dist):
    ...
    left_bottom = [img_x_center-470, imshape[0]]
    right_bottom = [img_x_center+470, imshape[0]]
...
def pipeline(image):
    ...
    grad_binary = abs_sobel_thresh(undist, orient='x', sobel_kernel=7, thresh=(50,150))
    mag_binary = mag_thresh(undist, sobel_kernel=7, mag_thresh=(50, 150))
    dir_binary = dir_threshold(undist, sobel_kernel=7, thresh=(0.5, 1.3))
    hls_binary = hls_select(undist,thresh=(120, 255))

    ksize = 7
    # Choose a larger odd number to smooth gradient measurements
    gradx = abs_sobel_thresh(undist, orient='x', sobel_kernel=ksize, thresh=(50, 200))
    ...
    #    else:
#        print("Entering NOT window-sliding ")
...
#
    ...
    cv2.putText(result,'Distance from center:'+str(round(center_dist,3)), (25,225), cv2.FONT_ITALIC, 2, (0,0,0),3 )
